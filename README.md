# 👨‍💻 Jorge Israel Barreto Matute

**Tecnólogo en Desarrollo de Software | Full-Stack Developer & IT Support**

Profesional enfocado en el ciclo de vida de aplicaciones, desarrollo de APIs REST y resolución de problemas de infraestructura. Combino el desarrollo Full-Stack con experiencia práctica en recuperación de desastres, administración Linux y despliegue de contenedores.

---

## 🚀 Tecnologías y Herramientas

* **Backend:** Java (Spring Boot, Spring Security), Python
* **Frontend:** Angular, React, Flutter
* **Infraestructura & Cloud:** Docker, GCP (IaaS), Linux (Shell Scripting)
* **Base de Datos & Datos:** SQL, Dagster

---

## 🏆 Proyectos Destacados

### 🏨 [Turismo Tec](Enlace_a_tu_repo)
Plataforma SPA para la gestión de servicios turísticos.
* **Stack:** Java Spring Boot, Angular, GCP, Docker.
* **Aporte:** Desarrollo de API REST con +190 endpoints e integración de IA (OpenAI) y pasarela de pagos (PayPal). Despliegue en producción mediante contenedores en servidores Ubuntu (GCP).

### 🔬 [Sistema de Gestión de Inventario para Laboratorios](Enlace_a_tu_repo)
Sistema centralizado para auditar hardware, mantenimientos preventivos y control de repuestos.
* **Stack:** Angular 18, Spring Boot 3.3.2, MySQL, SendGrid API.
* **Aporte:** Implementación de control de acceso por roles (TIC, Encargados), reportes operativos y notificaciones automatizadas por correo.

### 🔒 [API Facturación Segura & Microservicio Criptografía](Enlace_a_tu_repo)
Sistemas enfocados en la seguridad de la información y transacciones.
* **Stack:** Spring Boot 3.5.3, Docker, JWT, OpenAPI (Swagger).
* **Aporte:** Arquitectura stateless con autenticación JWT, encriptación AES-128/RSA-2048, filtros IP (Whitelist) y orquestación con Docker Compose.

### 📊 [Pipeline de Datos COVID-19](Enlace_a_tu_repo)
Orquestación de datos epidemiológicos para análisis y reportes.
* **Stack:** Python, Dagster.
* **Aporte:** Automatización de ingesta, cálculo de métricas (incidencia, factor de crecimiento), validación de assets y generación de reportes en Excel.

---

## 💼 Experiencia Técnica

* **Desarrollador Full-Stack** | *Turismo Tec* (2024-2025): Arquitectura backend/frontend y despliegue en la nube.
* **Soporte Técnico y de Software** | *GAD Santa Isabel* (2024): Corrección de bugs críticos (Angular/Spring Boot) y despliegue de servidor local (Ubuntu).
* **Especialista en Soporte TI** | *Independiente, Cuenca* (2018-2026): Data recovery, reparación de sistemas Linux por desastres eléctricos y optimización de hardware.

---

## 📬 Contacto

* 📧 Email: [jorge.barreto.97@gmail.com](mailto:jorge.barreto.97@gmail.com)
* 🌐 Web: [thebigg7.github.io](https://thebigg7.github.io)
* 💼 LinkedIn: [Tu URL de LinkedIn]
